"""
Reel D -- Ushtrimi 2, Njesia 4.8A
y = x^2 + 2,  y = 0,  x = 0,  x = 3   ->  integral_0^3 (x^2+2) dx = 15

Standalone vertical reel: hook graph (parabola!), solve, answer + CTA.
"""
...
class ReelD(Scene):
    ...
    def hook(self):
        axes = Axes(
            x_range=[-0.5, 4, 1], y_range=[0, 12, 2],
            x_length=5, y_length=3.5,
            axis_config={"include_numbers": True, "font_size": 20, "color": DIVIDER_COLOR},
        )
        axes.move_to(UP * 1.8)

        # Plot y = x^2 + 2  (parabola)
        graph = axes.plot(lambda x: x**2 + 2, x_range=[-0.3, 3.3], color=SHAPE_COLOR, stroke_width=3)

        # Shaded area between x=0 and x=3
        area = axes.get_area(graph, x_range=[0, 3], color=SHAPE_COLOR, opacity=0.35)

        # Boundary dashed line at x=3 (x=0 is the y-axis)
        x2_line = DashedLine(
            axes.c2p(3, 0), axes.c2p(3, 11),
            color=LABEL_COLOR, stroke_width=2, dash_length=0.08,
        )

        func_label = MathTex(r"y = x^2 + 2", font_size=SMALL_SIZE, color=SHAPE_COLOR)
        func_label.next_to(axes.c2p(3.2, 11), RIGHT, buff=0.15)

        x2_label = MathTex(r"x=3", font_size=20, color=LABEL_COLOR)
        x2_label.next_to(axes.c2p(3, 0), DOWN, buff=0.25)
        ...
        self.play(Create(graph), run_time=0.7)
        self.play(
            FadeIn(x2_line), FadeIn(x2_label),
            run_time=0.4,
        )
        self.play(DrawBorderThenFill(area), run_time=0.8)
    ...
    def solve(self):
        ...
        step1 = MathTex(
            r"= \left[ \frac{x^3}{3} + 2x \right]_{0}^{3}",
            font_size=EQ_SIZE, color=WHITE,
        )
        ...
        step2 = MathTex(
            r"= \left( \frac{3^3}{3} + 2 \cdot 3 \right) - \left( \frac{0^3}{3} + 2 \cdot 0 \right)",
