"""
Reel C -- Ushtrimi 2, Njesia 4.8A
y = 2x + 1,  y = 0,  x = 2,  x = 3   ->  integral_2^3 (2x+1) dx = 6
...
class ReelC(Scene):
    ...
    def hook(self):
        ...
        # Plot y = 2x + 1
        graph = axes.plot(lambda x: 2 * x + 1, x_range=[0, 3.5], color=SHAPE_COLOR, stroke_width=3)

        # Shaded area between x=2 and x=3
        area = axes.get_area(graph, x_range=[2, 3], color=SHAPE_COLOR, opacity=0.35)

        # Boundary dashed lines
        x1_line = DashedLine(
            axes.c2p(2, 0), axes.c2p(2, 5),
            color=LABEL_COLOR, stroke_width=2, dash_length=0.08,
        )
        x2_line = DashedLine(
            axes.c2p(3, 0), axes.c2p(3, 7),
            color=LABEL_COLOR, stroke_width=2, dash_length=0.08,
        )

        func_label = MathTex(r"y = 2x + 1", font_size=SMALL_SIZE, color=SHAPE_COLOR)
        func_label.next_to(axes.c2p(3.3, 7.5), RIGHT, buff=0.15)
