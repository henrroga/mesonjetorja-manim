"""
Reel A -- Ushtrimi 2, Njesia 4.8A
y = x,  y = 0,  x = 1,  x = 2   ->  integral_1^2 x dx = 3/2 = 1,5
...
class ReelA(Scene):
    ...
    def hook(self):
        ...
        # Plot y = x
        graph = axes.plot(lambda x: x, x_range=[0, 2.8], color=SHAPE_COLOR, stroke_width=3)

        # Shaded area between x=1 and x=2
        area = axes.get_area(graph, x_range=[1, 2], color=SHAPE_COLOR, opacity=0.35)

        # Boundary lines x=1, x=2 (dashed)
        x1_line = DashedLine(
        ...
        func_label.next_to(axes.c2p(2.6, 2.6), RIGHT, buff=0.15)

        x1_label = MathTex(r"x=1", font_size=20, color=LABEL_COLOR)
        x1_label.next_to(axes.c2p(1, 0), DOWN, buff=0.25)
        x2_label = MathTex(r"x=2", font_size=20, color=LABEL_COLOR)
        x2_label.next_to(axes.c2p(2, 0), DOWN, buff=0.25)

        graph_group = VGroup(axes, graph, area, x1_line, x2_line, func_label, x1_label, x2_label)
