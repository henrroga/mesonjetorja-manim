"""
Reel E — Ushtrimi 2, Njësia 6.2A
Matematika 12 (Botime Pegi)

Vektori e) 3i - 5j  →  |v| = √34 ≈ 5,83;  α ≈ -59,0° (Q4, y < 0 → këndi negativ)
"""
...
class ReelE(Scene):
    ...
    def hook(self):
        ...
        # Component labels:
        # x-component "3" ABOVE horizontal dashed line (since vy < 0)
        x_lbl = MathTex("3", font_size=22, color=LABEL_COLOR)
        x_lbl.next_to(h_line, UP, buff=0.12)
        # y-component "-5" to the RIGHT of vertical dashed line
        y_lbl = MathTex("-5", font_size=22, color=LABEL_COLOR)
        y_lbl.next_to(v_line, RIGHT, buff=0.12)

        # Right-angle mark at (3, 0)
        tow_o = np.array(origin) - np.array(h_end)
        n1 = np.linalg.norm(tow_o)
        tow_o = tow_o / n1 if n1 > 0 else LEFT
        ...
        angle_rad = np.arctan2(-5, 3)  # ≈ -1.030 rad ≈ -59.0°
        arc = Arc(
            radius=0.45,
    ...
    def solve(self):
        ...
        cos_eq = MathTex(
            r"\alpha = -\cos^{-1}\!\Big(\frac{3}{5{,}83}\Big)",
            font_size=EQ_SIZE, color=WHITE,
        )
