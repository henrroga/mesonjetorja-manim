"""
Reel A — Ushtrimi 2, Njësia 6.2A
Matematika 12 (Botime Pegi)

Vektori a) 5i + 2j  →  |v| = √29 ≈ 5,39;  α ≈ 21,8°
...
class ReelA(Scene):
    ...
    def hook(self):
        ...
        # Component labels
        x_lbl = MathTex("5", font_size=22, color=LABEL_COLOR)
        x_lbl.next_to(h_line, DOWN, buff=0.12)
        y_lbl = MathTex("2", font_size=22, color=LABEL_COLOR)
        y_lbl.next_to(v_line, RIGHT, buff=0.12)

        # Angle arc
        angle_rad = np.arctan2(2, 5)
        arc = Arc(
            radius=0.6, start_angle=0, angle=angle_rad,
            arc_center=origin, color=HIGHLIGHT_COLOR, stroke_width=2,
        )
        alpha_lbl = MathTex(r"\alpha", font_size=22, color=HIGHLIGHT_COLOR)
        mid_a = angle_rad / 2
        alpha_lbl.move_to(origin + 0.9 * np.array([np.cos(mid_a), np.sin(mid_a), 0]))

        # Right-angle mark at (5, 0)
        tow_o = origin - h_end
    ...
    def solve(self):
        ...
        cos_eq = MathTex(
            r"\alpha = \cos^{-1}\!\Big(\frac{5}{5{,}39}\Big)",
            font_size=EQ_SIZE, color=WHITE,
        )
