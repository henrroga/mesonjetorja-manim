"""
Reel F — Ushtrimi 2, Njësia 6.2A
Matematika 12 (Botime Pegi)

Vektori f) -6i - 5j  →  |v| = √61 ≈ 7,81;  α ≈ -140,2° (Q3, y < 0 → këndi negativ)
"""
...
class ReelF(Scene):
    ...
    def hook(self):
        ...
        # Dashed components: origin → (-6,0) → (-6,-5)
        h_end = axes.c2p(-6, 0)
        h_line = DashedLine(
        ...
        # Component labels:
        # x-component "-6" ABOVE horizontal (since vy < 0, triangle is below)
        x_lbl = MathTex("-6", font_size=22, color=LABEL_COLOR)
        x_lbl.next_to(h_line, UP, buff=0.12)
        # y-component "-5" to the LEFT of vertical (since vx < 0)
        y_lbl = MathTex("-5", font_size=22, color=LABEL_COLOR)
        y_lbl.next_to(v_line, LEFT, buff=0.12)

        # Right-angle mark at (-6, 0)
        tow_o = np.array(origin) - np.array(h_end)
        n1 = np.linalg.norm(tow_o)
        tow_o = tow_o / n1 if n1 > 0 else RIGHT
        tow_t = np.array(tip_pt) - np.array(h_end)
        n2 = np.linalg.norm(tow_t)
        tow_t = tow_t / n2 if n2 > 0 else DOWN
        sq_sz = 0.1
        ...
        # Angle arc from positive x-axis (clockwise, negative angle)
        angle_rad = np.arctan2(-5, -6)  # ≈ -2.447 rad ≈ -140.2°
        arc = Arc(
            radius=0.45,
            start_angle=angle_rad,
            angle=-angle_rad,  # sweep from negative angle back to 0
            arc_center=origin,
    ...
    def solve(self):
        ...
        cos_eq = MathTex(
            r"\alpha = -\cos^{-1}\!\Big(\frac{-6}{7{,}81}\Big)",
            font_size=EQ_SIZE, color=WHITE,
        )
