...
class Ushtrimi5(ExerciseScene):
    ...
    def part_a(self):
        ...
        # ────────────────────────────────────────────
        s1t = self.centered_title("Hapi 1: Rishkruajme fuqite", y_pos=3.2)

        s1txt = self.centered_text([
        ...
        # ────────────────────────────────────────────
        s2t = self.centered_title("Hapi 2: Zevendesojme ne ekuacion", y_pos=3.2)

        s2txt = self.centered_text([
            r"\text{Ekuacioni behet:}",
        ], s2t)
        self.wait(1.5)

        s2eq1 = self.centered_eq(
            r"\frac{1}{3}(3^x)^2 - \frac{5}{3} \cdot 3^x + 2 = 0",
            s2txt, buff=0.35, key=True, font_size=34,
        )

        s2txt2 = self.centered_text([
            r"\text{Shumezojme te dyja anet me 3}",
            r"\text{per te hequr thyesat:}",
        ], s2eq1, buff=0.3)
        self.wait(2.0)

        s2eq2 = self.centered_eq(
            r"(3^x)^2 - 5 \cdot 3^x + 6 = 0",
            s2txt2, buff=0.3, color=LABEL_COLOR, key=True, font_size=36,
        )
        self.wait(1.5)
        ...
        # ────────────────────────────────────────────
        s3t = self.centered_title("Hapi 3: Zevendesimi", y_pos=3.2)

        s3txt = self.centered_text([
            r"\text{Kjo eshte nje teknike standarde:}",
            r"\text{zevendesojme } t = 3^x",
        ], s3t)
        self.wait(2.5)

        s3eq1 = self.centered_eq(
            r"\text{Le } t = 3^x, \quad t > 0",
            s3txt, buff=0.35, color=HIGHLIGHT_COLOR, font_size=34, key=True,
        )

        s3eq2 = self.centered_eq(
            r"t^2 - 5t + 6 = 0",
            s3eq1, buff=0.35, color=LABEL_COLOR, font_size=38, key=True,
        )
        self.wait(1.5)
        ...
        # ────────────────────────────────────────────
        s4t = self.centered_title("Hapi 4: Faktorizimi", y_pos=3.2)

        s4eq0 = self.centered_eq(
            r"t^2 - 5t + 6 = 0",
            s4t, buff=0.35, font_size=34,
        )

        s4txt = self.centered_text([
            r"\text{Gjejme dy numra qe shumezojne 6}",
            r"\text{dhe mbledhin 5: keta jane 2 dhe 3}",
        ], s4eq0, buff=0.3)
        self.wait(3.0)

        s4eq1 = self.centered_eq(
            r"(t - 2)(t - 3) = 0",
            s4txt, buff=0.3, color=LABEL_COLOR, font_size=36, key=True,
        )

        s4eq2 = self.centered_eq(
            r"t_1 = 2 \qquad t_2 = 3",
            s4eq1, buff=0.35, color=ANSWER_COLOR, font_size=36, key=True,
        )
        self.wait(1.5)
        ...
        # ────────────────────────────────────────────
        s5t = self.centered_title("Hapi 5: Kthehemi te x", y_pos=3.2)

        s5txt = self.centered_text([
    ...
    def part_b(self):
        ...
        # ────────────────────────────────────────────
        s1t = self.panel_title("Hapi 1: Barazojme grafiket", y_pos=3.0)
        ...
        s1eq1 = self.panel_eq(
            ...
            s1txt, buff=0.3, key=True, font_size=30,
        )
        ...
        s1eq2 = self.panel_eq(
            r"3^{2x-1} - 5 \cdot 3^{x-1} + 2 = 0",
            s1txt2, buff=0.25, color=LABEL_COLOR, font_size=28, key=True,
        )
        ...
        # ────────────────────────────────────────────
        s2t = self.panel_title("Hapi 2: Zgjidhjet nga pika a)", y_pos=3.0)
        ...
        s2eq1 = self.panel_eq(
            r"x_1 = 1",
            s2txt, buff=0.3, color=ANSWER_COLOR, font_size=34, key=True,
        )

        s2eq2 = self.panel_eq(
            r"x_2 = \frac{\log 2}{\log 3} \approx 0{,}631",
            s2eq1, buff=0.3, color=ANSWER_COLOR, font_size=30, key=True,
        )
        self.wait(1.5)
        ...
        # ────────────────────────────────────────────
        s3t = self.panel_title("Hapi 3: Gjejme vlerat e y", y_pos=3.0)

        # y for x = 1
        s3txt1 = self.panel_text([
            r"\text{Kur } x = 1\text{:}",
        ], s3t)
        self.wait(1.0)
        ...
        s3eq1 = self.panel_eq(
            r"y = 5 \cdot 3^{1-1}",
            s3txt1, buff=0.25, font_size=30,
        )
        s3eq2 = self.panel_eq(
            r"y = 5 \cdot 3^0 = 5 \cdot 1",
            s3eq1, buff=0.2, font_size=30,
        )
        s3eq3 = self.panel_eq(
            r"y = 5",
            s3eq2, buff=0.2, color=ANSWER_COLOR, font_size=34, key=True,
        )
        ...
        self.transfer_value(s3eq3, VGroup(dot1, lbl1))
        self.wait(1.5)
        ...
        # y for x = log2/log3
        s3txt2 = self.panel_text([
            r"\text{Kur } x = \tfrac{\log 2}{\log 3}\text{:}",
        ], s3t, buff=0.3)
        self.wait(1.0)

        s3eq4_txt = self.panel_text([
        ...
        s3eq4 = self.panel_eq(
            ...
            s3eq4_txt, buff=0.25, font_size=30,
        )
        s3eq5 = self.panel_eq(
            r"y = 5 \cdot \frac{2}{3} = \frac{10}{3}",
            s3eq4, buff=0.2, font_size=30,
        )
        s3eq6 = self.panel_eq(
            r"y = \frac{10}{3} \approx 3{,}33",
            s3eq5, buff=0.2, color=ANSWER_COLOR, font_size=34, key=True,
        )
        ...
        self.transfer_value(s3eq6, VGroup(dot2, lbl2))
        self.wait(2.0)
